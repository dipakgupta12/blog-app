...
export default function Post({ id }) {
  ...
  const [imageUrl, setImageUrl] = useState("https://enviragallery.com/wp-content/uploads/2016/05/Set-Default-Featured-Image.jpg")

  useEffect(() => {
    // Fetch data when the component mounts
    const data = fetchBlogDataDetails(id).then((result) => {
      setData(result?.blog);
      const base64ImageData = Buffer.from(result?.blog?.image?.data).toString("base64");
            ...
            <div className="p-6 relative w-full h-full">
              <div className='flex justify-between items-center'>
                ...
                <p className="text-sm text-gray-600 flex gap-2 items-center flex justify-end p-3">
                  <img className='w-10 h-10 rounded-full' alt="Blog author" src="https://img.freepik.com/premium-vector/business-global-economy_24877-41082.jpg" />
                  <span className='border-r flex items-center border-[#d1d1d1] pr-2 h-8'>{data.authorName}</span>
                  <img className='w-10 h-10 rounded-full' alt="Blog date" src="https://cdn-icons-png.flaticon.com/512/1869/1869397.png" />
                  {new Date(data.createdAt).toLocaleDateString("en-US", dateFormat)}
                </p>
              </div>
